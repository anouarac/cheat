...
TO_TRAIN = 0
if TRAIN_MODE:
    TO_TRAIN = int(input("Model to train: (1: Q-Agent, 2: RNN) "))
iterations, cur = 1, 1
if TO_TRAIN == 1:
...
if TO_TRAIN == 2:
    ...
    while cur <= iterations:
        ...
        window = Window(state)
        players = [Player(random.randint(6, 6)) for i in range(nbr_players)]
        if TRAIN_MODE:
            for i in range(nbr_players // 2):
                players[randint(0, nbr_players - 1)] = Player(randint(5, 6))
        else:
            ...
            while not state.ended:
                ...
                if players[state.turn].type == 0 and MODE == 0:
                    ...
                    if inp == "show":
                        ...
                        idrep = 2
                        strings = list(
                            input("Select your cards: (e.g. 1D 3C KS) ").split(" ")
                        )
                        for s in strings:
